...
This Agentic AI system demonstrates intelligent tool selection and autonomous decision-making capabilities:

### **🤖 Core Features:**
- **Single-Tool Selection**: Automatically chooses ONE tool (Tavily OR Wikipedia) or uses no tools
- **Smart Decision Making**: AI determines the most appropriate tool based on query type
- **Tool Usage Tracking**: Real-time display of which tools were used
- **Agentic Behavior**: Autonomous tool selection without user intervention

### **🛠️ Technology Stack:**
- **LLM**: Groq (llama3-70b-8192)
- **Tools**: Tavily Search (current events) + Wikipedia (historical facts)
- **Framework**: LangChain + LangGraph
- **Frontend**: Streamlit
- **Backend**: FastAPI
- **Deployment**: AWS ECS Fargate with Jenkins CI/CD

### **🎯 Use Cases:**
- **Current Events**: Uses Tavily for recent news and real-time information
- **Historical Facts**: Uses Wikipedia for established knowledge and concepts
- **Conversational**: Uses no tools for opinion-based or direct questions
...
```bash
git clone https://github.com/data-guru0/multi_aai_jk_fg_sq.git
cd multi_aai_jk_fg_sq
```

> **Note**: The Jenkinsfile is already configured with the correct repository URL.
...
- ✅ **GitHub Setup Completed**  
  ...
  - `sample_env.txt` template provided for environment variables
...
2. The Jenkinsfile includes multiple stages:
   - **Checkout**: Fetch latest code from GitHub
   - **SonarQube Analysis**: Code quality analysis
   - **Build & Push to ECR**: Docker image build and push to AWS ECR
   - **Deploy to ECS**: Automated deployment to AWS Fargate

> **Explanation**: This comprehensive pipeline automates the entire CI/CD process for the Agentic AI project.
...
### 8. Configure Environment Variables for Agentic AI
...
4. Add the following environment variables:
   - **GROQ_API_KEY:** `gsk_...` (for Groq LLM access)
   - **TAVILY_API_KEY:** `tvly-dev-...` (for Tavily search access)
5. Save the changes and **redeploy the task**.

> **Note**: These environment variables are required for the Agentic AI to function properly with tool access.

---

### ✅ **Deployment Complete**

Your Agentic AI application is now deployed to AWS ECS Fargate. You can access it via the public IP at port `8501`. The deployment process has been automated using Jenkins, and the Agentic AI system is now live with full tool selection capabilities.
